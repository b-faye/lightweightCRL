...
class BahdanauAttention(nn.Module):
    def __init__(self, input_dim=CFG.projection_dim, *args, **kwargs):
        """
        Initialize BahdanauAttention module.
        ...
        self.value_projection = nn.Linear(self.input_dim, self.input_dim)  # Linear transformation for values
        self.context_projection = nn.Linear(self.input_dim, self.input_dim)  # Linear transformation for contexts
        self.linear_projection = nn.Linear(self.input_dim, 1)  # Linear transformation for computing attention scores

    def forward(self, x):
        """
        Forward pass of the BahdanauAttention module.

        Args:
            x (tuple): Tuple containing values and contexts.
                       values: Tensor of shape (batch_size, sequence, projection_dim).
                       contexts: Tensor of shape (batch_size, projection_dim).

        Returns:
            bahdanau_contexts (Tensor): Context vector computed using Bahdanau attention mechanism.
        ...
        scores = self.linear_projection(self.context_projection(contexts) + self.value_projection(values))
        # Compute attention weights using softmax
        attention_weights = F.softmax(scores, dim=1)
        # Compute Bahdanau contexts
        bahdanau_contexts = attention_weights * values
        # Sum along the sequence dimension to obtain the final context vector
...
class Fusion(nn.Module):
    ...
    def forward(self, inputs):
        ...
        ## Fusion block
        attention = self.bahdanau_attention([x, contexts])
